...
//images/simplefilemanager/56eb3fc274a821.96089346/Dongle_Oled_rev3.cpp.hex
    ...
    socketServer.emit("send_to_client", {data: {apioId: req.body.apioId, value: true}, message: "dongle_onoff_update"});
});
...
Apio.Serial.stream = function (data, callback) {
    ...
    var arr = keys[0].split("[");
    var address = arr[arr.length -1].split("]")[0];
    Apio.Database.db.collection("Objects").findOne({objectId: data.objectId}, function (err, doc) {
        if (err) {
            ...
            //MODIFICARE ADDRESS, ANDARE PER ESCLUSIONE NEL CASO IN CUI PROPERTY NON HA ADDRESS
            if (!isNaN(address)) {
                data.address = address;
            } else {
                if (doc.address instanceof Array) {
                    var temp = data.address;
                    var keys = Object.keys(doc.properties);
                    for (var i = 0; data.address === temp && i < doc.address.length; i++) {
                        var found = false;
                        var regex = new RegExp("\[" + doc.address[i] + "\]$");
            ...
            keys.forEach(function (key) {
                ...
                doTheStreaming(data.protocol, data.address, f, data.properties[key], function (err) {
                })
            })
        }
    })
        ...
        var packageMessageAndAddToQueue = function (protocol, address, key, value, callback) {
            ...
            switch (protocol) {
                case "e":
                    ...
                    console.log("devo rigirare sta roba a Enocean Service ", o);
                ...
                default:
                    if (fs.existsSync(__dirname + "/public/applications/" + data.objectId + "/adapter.js")) {
                        ...
                        ////console.log("Apio.Serial.Send() Error: protocol " + data.protocol + "is not supported and no adapter was found.");
                    }
                    break;
            }
        }
            ...
            if (data && Apio.Database.db) {
                ...
                Apio.Database.db.collection("Objects").findOne({objectId: data.objectId}, function (err, doc) {
                    if (err) {
                        ...
                        var numberOfKeys = keys.length;
                        var available = true;
                        //commentato perchè sembre servisse ad inviare mail prima del service mail sms e notification
                        ...
                        keys.forEach(function (key) {
                            ...
                            packageMessageAndAddToQueue(data.protocol, data.address, f, data.properties[key], function (err) {
                            });
                        })
                        //}
...
Apio.Serial.read = function (data) {
    ...
    if (Apio.Database.db) {
        ...
        data.address = data.objectId;
        Apio.Database.db.collection("Objects").find({address: data.objectId}).toArray(function (err, objects) {
            if (err) {
                console.log("Error while getting objects with address " + data.objectId + ": ", err);
            } else if (objects) {
                objects.forEach(function(res) {
                    ...
                    switch (command) {
                                ...
                                Apio.Database.db.collection("Objects").findOne({objectId: data.objectId}, function (err, res) {
                                    if (err) {
                                        ...
                                        for (var i in res.properties) {
                                            //MODIFICARE
                                            if (res.address instanceof Array) {
                                                var arr = i.split("[" + data.address + "]");
                                                var prop = arr[0];
                                                for (var g = 1; g < arr.length - 1; g++) {
                                                    prop += "[" + data.address + "]";
                                                }
                                                if (data.properties[prop] !== undefined && typeof data.properties[prop] !== "object" && data.properties[prop] !== null && data.properties[prop] !== "") {
                                                    if (!isNaN(String(data.properties[prop]).replace(",", "."))) {
                                                        updt["log." + i + "." + timestamp] = data.properties[prop];
                                                    }
                                        ...
                                        //MODIFICARE
                                        if (res.address instanceof Array) {
                                            var resPropertiesKeys = Object.keys(res.properties);
                                            var propertiesKeys = Object.keys(data.properties);
                                            for (var i in propertiesKeys) {
                                                if (resPropertiesKeys.indexOf(propertiesKeys[i] + "[" + data.address + "]") > -1) {
                                                    data.properties[propertiesKeys[i] + "[" + data.address + "]"] = JSON.parse(JSON.stringify(data.properties[propertiesKeys[i]]));
                                                    delete data.properties[propertiesKeys[i]];
                                                }
                                            }
                                        }
                        ...
                        case "hi":
                            ...
                            if (data.objectId == "9999") {
                                ...
                                //mettere in un array tutte le stringhe inviate per evitare di mandare due volte
...
/*process.on("uncaughtException", function (err) {
 ...
 //Apio.Serial.available = true;
 Apio.Serial.closeDongle(function (err) {
