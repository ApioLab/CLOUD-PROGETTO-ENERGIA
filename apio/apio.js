        ...
        if (!Apio.hasOwnProperty("io")) {
            ...
            Apio.io.on("connection", function (socket) {
                ...
                socket.on("apio.create.new.app", function (data) {
                    ...
                    Apio.Database.db.collection("Objects").insert(mongo, function (error) {
                        if (error) {
                ...
                socket.on("apio.server.log.sync", function (data) {
                        ...
                        sql_db.connect(function (err) {
                            if (err) {
                                ...
                                for (var i in data.logs) {
                                    index++;
                                    var rows = [];
                                    var fields = Object.keys(data.logs[i][0]);
                                    for (var j in data.logs[i]) {
                                        var row = [];
                                        for (var k in data.logs[i][j]) {
                                            row[fields.indexOf(k)] = data.logs[i][j][k];
                                        }
                                    ...
                                    var sql = "INSERT INTO `" + i + "_" + data.apioId + "` (" + fields.join() + ") VALUES ?";
                                    sql_db.query(sql, [rows], function (s_err) {
                                        if (s_err) {
                                            console.log("Error while inserting data in table " + i + "_" + data.apioId + ": ", s_err);
                                        } else {
                                            console.log("Successfully inserted data " + rows + " in table " + i + "_" + data.apioId);
                                            index--;
                                            if (index === 0) {
                                                sql_db.end();
                                            }
                ...
                socket.on("serial_update", function (data) {
                    ...
                    if (Apio.Configuration.type === "gateway") {
                        ...
                        Apio.Communication.sendAddressBindToProperty(data.command, data.address, data.properties, data.apioId, objects[data.objectId].protocol);
                ...
                socket.on("apio_client_update", function (data) {
                    ...
                    Apio.Object.update(data, function () {
                        if (Apio.Configuration.type === "gateway") {
                            ...
                            if (Apio.connectedSockets.hasOwnProperty(data.apioId) && Apio.connectedSockets[data.apioId][0]) {
                                var socketId = Apio.connectedSockets[data.apioId][0];
        ...
        MongoClient.connect(Apio.Database.getConnectionString(), function (error, db) {
            ...
            Apio.Database.db.collection("Objects").find().toArray(function (err, doc) {
                if (err) {
                    ...
                    console.log("-----------------OBJECTS VALE: ", objects);
                }
            });
            ...
            Apio.Database.db.collection("Objects").findOne({objectId: id}, function (err, obj) {
                if (err) {
                    ...
                    for (var p in protocols) {
                            ...
                            for (var prop in properties) {
                                if (typeof Apio.addressBindToProperty[protocols[p]][addresses[a]][properties[prop]] === "object") {
                                    if (Apio.addressBindToProperty[protocols[p]][addresses[a]][properties[prop]].hasOwnProperty(id)) {
                                        ...
                                        if (Apio.servicesSocket.hasOwnProperty(protocols[p])) {
                                            Apio.servicesSocket[protocols[p]].emit("update_collections", {
                                                command: "delete",
                                                deleteData: {
                                                    protocol: protocols[p],
                                                    address: addresses[a],
                                                    originalProperty: properties[prop],
                                                    bindedObjectId: id
                                                }
                                            });
                                        }
                        ...
                        if (Object.keys(Apio.addressBindToProperty[protocols[p]]).length === 0) {
                            delete Apio.addressBindToProperty[protocols[p]];
                        }
                    }
    ...
    Apio.Object.update = function (data, callback, id) {
        ...
        if (data.writeToDatabase === true) {
            Apio.Database.updateProperty(data, function () {
                ...
                if (data && data.writeToSerial === true && Apio.Configuration.serial.enabled === true) {
                    Apio.Serial.send(data);
                }
            });
        }
    ...
    Apio.Serial.send = function (data) {
        ...
        if (!data.hasOwnProperty("protocol")) {
            for (var py in data.properties) {
                ...
                if (objects[data.objectId].properties[py] && objects[data.objectId].properties[py].hasOwnProperty("protocol") && objects[data.objectId].properties[py].protocol.hasOwnProperty("address") && objects[data.objectId].properties[py].protocol.hasOwnProperty("name")) {
                    var p = {};
                    for (var i in objects) {
                        if (objects[i].address === objects[data.objectId].properties[py].protocol.address) {
                            var p = {};
                            p[objects[data.objectId].properties[py].protocol.property] = data.properties[py];
                            break;
                        }
                    }
                    console.log("Apio.Serial.send prima di Apio.Communication.sendAddressBindToProperty", objects[data.objectId].properties[py].protocol.address);
                    Apio.Communication.sendAddressBindToProperty("", objects[data.objectId].properties[py].protocol.address, p, data.apioId, data.objectId, objects[data.objectId].properties[py].protocol.name);
                } else if (py !== "date" && Apio.addressBindToProperty.hasOwnProperty(protocol) && Apio.addressBindToProperty[protocol].hasOwnProperty(data.address) && Apio.addressBindToProperty[protocol][data.address].hasOwnProperty(py)) {
