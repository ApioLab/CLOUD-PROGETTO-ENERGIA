...
socketServer.on("connection", function (socket) {
    ...
    if (socket.handshake.address.indexOf("127.0.0.1") === -1) {
                    ...
                    if (!interval) {
                        interval = setInterval(function () {
                            if (Object.keys(boardsToSync).length) {
                                ...
                                for (var i in keys) {
                                    if (new Date().getTime() - boardsToSync[keys[i]].timestamp >= 5 * 60 * 1000) {
        ...
        socket.on("apio_board_has_been_enabled", function (data) {
            ...
            //boardsToSync[data.body.apioId].socket.emit("apio.remote.sync.request", data.body);
            delete boardsToSync[data.body.apioId];

            console.log("boardsToSync: ", boardsToSync);
        });

        socket.on("apio_ask_boards_to_sync", function () {
            //console.log("apio_get_boards_to_sync: ", boardsToSync);
            //socket.emit("apio_get_boards_to_sync", Object.keys(boardsToSync));
            var j = {};
