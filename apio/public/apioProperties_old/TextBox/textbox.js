    ...
    return {
        ...
        link: function (scope, elem, attrs) {
            ...
            socket.on("apio_server_update", function (self) {
                if (self.apioId === scope.object.apioId && self.objectId === scope.object.objectId && !currentObject.isRecording()) {
                    if (self.properties.hasOwnProperty(attrs["propertyname"])) {
                        ...
                        else if (scope.model !== self.properties[attrs["propertyname"]]) {
            ...
            scope.$watch("object.properties." + attrs["propertyname"], function () {
                if (scope.model !== scope.object.properties[attrs["propertyname"]]) {
                ...
                $(elem).find("textarea").on("blur", function () {
                    ...
                    if (!currentObject.isRecording()) {
                        ...
                        if (attrs["listener"]) {
                            ...
                            currentObject.update(attrs["propertyname"], scope.model, true, writeToSerial);
                            scope.$parent.object.properties[attrs["propertyname"]] = scope.model;
                        }
                        //
